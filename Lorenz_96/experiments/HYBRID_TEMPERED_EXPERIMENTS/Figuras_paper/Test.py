...
for inat , nat in enumerate( Nature_name ) :   
   ...
   for iiter in range( np.size(rip_range) ) :
       rmse_min_trip_etkf[iiter]    = np.min( rmse_trip[:,0,iiter] )  #Tomo el minimio considerando solo el bridge parameter =0.
       rmse_min_trip_hib[iiter]     = np.min( rmse_trip[:,1:,iiter])  #Tomo el minimo sobre todos los valores de bridge parameter excepto el 0.
       rmse_min_adtrip_etkf[iiter]  = np.min( rmse_adtrip[:,0,iiter] )  #Tomo el minimio considerando solo el bridge parameter =0.
       rmse_min_adtrip_hib[iiter]   = np.min( rmse_adtrip[:,1:,iiter])  #Tomo el minimo sobre todos los valores de bridge parameter excepto el 0.   
