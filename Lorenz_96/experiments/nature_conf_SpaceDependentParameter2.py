...
#In this experiment we activate the parameter random walk. This generates a time
#dependent parameter that changes according to an AR1 process.
#The amplitude and smoothness of the time dependence for the parameter can be controlled
#using CSigma and CPhi, the random term and autocorrelation of the AR1 process.
...
GeneralConf['ExpName']='SpaceDependentParameter2'          #Experiment name.
...
ModelConf['FSpaceDependent']=True                       #If the forcing parameters will depend on the location.
ModelConf['FSpaceAmplitude']=np.array([0.5])            #Amplitude of space variantions (for each coefficient)
ModelConf['FSpaceFreq']     =np.array([5])             #Use integers >= 1
...
ModelConf['CPhi'  ]=0.0                                  #Parameter random walk phi
...
ObsConf['Freq']=4                                    #Observation frequency in model time units (will also control nature run output frequency)
