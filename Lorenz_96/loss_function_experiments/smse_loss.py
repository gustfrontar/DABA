...
#=================================================================
# DEFINE AND TRAIN MODEL (RMSE) 
#=================================================================

rmse_model = ml_model.get_model( nx , expand_factor )

rmse_model = ml_model.initialize_weights( rmse_model )

rmse_model , rmse_Verification = ml_model.train_model( rmse_model , learning_rate , TrainDataloader , TestDataloader , max_epochs , loss = 'SMSE' ) 
...
ValInput , ValOutput , ValTarget = ml_model.model_eval( rmse_model , ValDataloader )
...
plt.plot( 0.5 * (bins[1:] + bins[:-1]) , np.log(freq_rmse) , 'r-', label='RMSE loss')
plt.xlabel('X')
plt.title('Log frequency')
plt.show(block=False)     
