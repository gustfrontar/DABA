...
da_exp['exp_id']='EMD_xyz'    #Este es un identificador que se agregara al nombre de los archivos generados por este script (figuras, pickle contiendo los datos, etc)
...
da_exp['rtps_alpha'] = 0.6   #Relaxation to prior spread (Whitaker y Hamill 2012)
da_exp['rejuv_param'] = 0.4  #Parametro de rejuvenecimiento (Acevedo y Reich 2017)
...
for i in tqdm( range(1,da_exp['numstep']) ) :
    ...
    [ da_exp['stateaens'][i,:,:] , da_exp['statea'][i,:] , da_exp['Pa'][i,:,:] , da_exp['OmB'][i,:] , da_exp['OmA'][i,:] , da_exp['w'][i,:] , S ] =da.analysis_update_EMD(da_exp['yobs'][i,:],da_exp['statefens'][i,:,:,0],forward_operator,da_exp['R'] ,da_exp['rtps_alpha'] , da_exp['rejuv_param'])
