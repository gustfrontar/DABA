...
outfile_prefix = './npz/Sesitivity_experiment_loc_multinf_adtemp_LETKF_'
...
for Nature in NatureList :
    ...
    loc_range = np.arange(1,4.5,0.5)
    temp_range = np.arange(1,4,1)

    total_analysis_rmse = np.zeros( (len(mult_inf_range),len(loc_range),len(temp_range)) )
    total_analysis_sprd = np.zeros( (len(mult_inf_range),len(loc_range),len(temp_range)) )
    total_forecast_rmse = np.zeros( (len(mult_inf_range),len(loc_range),len(temp_range)) )
    total_forecast_sprd = np.zeros( (len(mult_inf_range),len(loc_range),len(temp_range)) )
    ...
    for iinf , mult_inf in enumerate( mult_inf_range ) :
        for iloc , loc in enumerate( loc_range )  :
            for itemp , temp in enumerate( temp_range )       :
                ...
                conf.DAConf['LocScalesLETKF']=np.array([loc,-1])
                conf.DAConf['LocScalesLETPF']=np.array([loc,-1])
                ...
                total_analysis_rmse[iinf,iloc,itemp] = np.mean(results[-1]['XASRmse'])
                total_forecast_rmse[iinf,iloc,itemp] = np.mean(results[-1]['XFSRmse'])
                total_analysis_sprd[iinf,iloc,itemp] = np.mean(results[-1]['XASSprd'])
                total_forecast_sprd[iinf,iloc,itemp] = np.mean(results[-1]['XFSSprd'])
        
    f=open(out_filename,'wb')
    pickle.dump([results,mult_inf_range,loc_range,temp_range,total_analysis_rmse,total_forecast_rmse,total_analysis_sprd,total_forecast_sprd],f)
    f.close()
