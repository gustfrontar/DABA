...
da_exp['exp_id']='GMDRLOCALH_xyz'    #Este es un identificador que se agregara al nombre de los archivos generados por este script (figuras, pickle contiendo los datos, etc)
...
da_exp['numstep']=500       # Cantidad de ciclos de asimilacion.
...
da_exp['bridge_parameter'] = 0.5 
da_exp['sample_size'] = 100
da_exp['beta_param']=0.2
da_exp['gamma_param']=0.0
...
for i in tqdm( range(1,da_exp['numstep']) ) :
    ...
    #[ da_exp['stateaens'][i,:,:] , da_exp['statea'][i,:] , da_exp['Pa'][i,:,:] , da_exp['OmB'][i,:] , da_exp['OmA'][i,:] ] =da.analysis_update_GMDR_localH(da_exp['yobs'][i,:],da_exp['statefens'][i,:,:,0],forward_operator,da_exp['R'],da_exp['MultInf'])
    tmp_for = np.copy(  da_exp['statefens'][i,:,:,0] )
    if da_exp['bridge_parameter'] < 1.0 :       
       R_loc = da_exp['R'] / (1.0 - da_exp['bridge_parameter'] )
       [ tmp_for , tmp_for_mean , null_var , null_var , null_var , wgm , kernel_perts ] =da.analysis_update_GM( da_exp['yobs'][i,:] , tmp_for , forward_operator , R_loc , da_exp['MultInf'] , beta_param = da_exp['beta_param'] , gamma_param = da_exp['gamma_param'] )
    if da_exp['bridge_parameter'] > 0.0 :
       R_loc = da_exp['R'] / ( da_exp['bridge_parameter'] )
       [ da_exp['stateaens'][i,:,:] , da_exp['statea'][i,:] , null_var , null_var , nul_var , tmp_sample ]= da.analysis_update_GMETPF( da_exp['yobs'][i,:] , tmp_for , wgm , kernel_perts , da_exp['sample_size'] , forward_operator , R_loc  ) 
