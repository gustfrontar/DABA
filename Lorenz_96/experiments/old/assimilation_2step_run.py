...
for ie in range(0,NEns)  :
   ...
   XA[:,ie,0]=XNature[:,0,0] + 2.0 * np.random.normal( size=Nx )
...
#   plt.figure()
#   plt.plot(np.squeeze(XNature[:,0,1]),'k');plt.plot(np.squeeze(XPF_MODE),'r');plt.plot(np.mean(XF[:,:,1],1),'b')
#
#   index=22
#   print(YObsW[index])
#   print(YF[index,:])
#   print(XF[index,:,1])
#   print(np.mean(XF[index,:,1],0))
#   print(XNature[index,:,0])
#   print(XPF_MODE[index])
#   print(WA[index,:])
   ...
   PseudoYObsW= np.copy(XPF_MEAN)
   PseudoObsErrorW=np.ones(np.shape(XPF_STD)) * 1.0   #Error of pseudo observations.
   ...
   #plt.figure()
   #plt.plot(np.squeeze(PseudoYObsW),'b');plt.plot(XNature[:,0,it],'k');plt.plot(np.mean(XA[:,:,it],1),'r')
   ...
   if DAConf['EstimateParameters']   : 
    ...
    if DAConf['ParameterLocalizationType'] == 1  :
       ...
       PA[:,:,:,it] = das.da_etkf( no=PseudoNObsW , nens=NEns , nvar=NCoef , xfens=PF[:,:,:,it] ,
                                            obs=PseudoYObsW, ofens=PseudoYF  , rdiag=PseudoObsErrorW ,
    ...
    if DAConf['ParameterLocalizationType'] == 2  :
       ...
       PA[:,:,:,it] = das.da_letkf( nx=Nx , nt=1    , no=PseudoNObsW , nens=NEns    ,  xloc=ModelConf['XLoc']        ,
                              tloc=da_window_end    , nvar=NCoef                    , xfens=PF[:,:,:,it]             ,
                              obs=PseudoYObsW       , obsloc=PseudoObsLocW          , ofens=PseudoYF                 ,
                              rdiag=PseudoObsErrorW , loc_scale=DAConf['LocScalesP'] , inf_coefs=DAConf['InfCoefsP'] ,
