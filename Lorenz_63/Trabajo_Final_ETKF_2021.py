...
# Trabajo final Curso DABA 2019
# Modificado por A. De Vera
...
da_exp['exp_id']='ETKF_adv'    #Este es un identificador que se agregara al nombre de los archivos generados por este script (figuras, pickle contiendo los datos, etc)
...
da_exp['forecast_length'] = 10                   # Plazo de pronostico (debe ser al menos 1)
da_exp['nvars'] = 3                              # Numero de variables en el modelo de Lorenz (no tocar)

da_exp['EnsSize'] = 20                           # Numero de miembros en el ensamble.
...
# Creamos un vector de bias para las observaciones.     #que no estan correlacionados entre si.
...
#%% 
#-------------------------------------------------------------------
# Graficamos el ensamble para alguna variable y ciclo en particular
#-------------------------------------------------------------------

import matplotlib.pyplot as plt

[forecast,analysis,nature] = da.get_forecast(da_exp,ini_time)

forecast_length = da_exp['forecast_length']
EnsSize = da_exp['EnsSize']
state = da_exp['state']
stateaens = da_exp['stateaens']
statefens = da_exp['statefens']
yobs = da_exp['yobs']
...
plt.figure()
for iens in range(da_exp['EnsSize']) :
   plt.plot(forecast[var,iens,:],'-',color='silver')
plt.plot(forecast[var,1,:],'-',color='silver',label = "Ens")
plt.plot(nature[var,:],'g',linewidth=2.5,label = "True")
#plt.plot(np.mean(forecast[var,:,:],0),'r-', label = "Ens Mean")
plt.plot(yobs[ini_time:ini_time+forecast_length,var],'ok', label = "Obs")
plt.title("Evolución de la variable 0 en el ciclo ini_time = 900")
...
#Graficamos la dispersion del ensamble como funcion del tiempo
#plt.figure()
...
#%% 
#------------------------------------------------
# "Recupero las variables que me interesan"
#------------------------------------------------

[numstep,nvars,EnsSize,forecast_length] = np.shape(da_exp['statefens'])

analysis = da_exp['statea']
nature   = da_exp['state']
yobs = da_exp['yobs']

forecast = np.zeros((nvars,EnsSize,forecast_length,numstep))

# get_forecast(da_exp,ini_time) devuelve el forecast inicializado en ini_time
# y el analisis y el nature que podemos usar para verificar dicho pronostico

for ini_time in range(numstep):
    [fcst,anl,true] = da.get_forecast(da_exp,ini_time)
    # fcst: el primer elemento es el analisis y los subsiguientes son los 
    # pronosticos a los diferentes plazos
    forecast[:,:,:,ini_time] = fcst

# RMSE y BIAS del analysis, first guess y observaciones
rmse_anl = da_exp['rmse_a']
rmse_firstguess = da_exp['rmse_f'][:,0]
rmse_obs = da_exp['rmse_o']
bias_anl = da_exp['bias_a']
bias_firstguess = da_exp['bias_f'][:,0]
bias_obs = da_exp['bias_o']
...
filename = 'DATApkl2mat'
outfile = open(filename,'wb') # write binary file
pickle.dump([analysis,nature,forecast],outfile)
outfile.close()
    
pickle.load(open('DATApkl2mat.pkl','rb'))  # read binary file

#scipy.io.savemat('data.mat', mdict={'data':data})  
scipy.io.savemat('DATApkl2mat_fcstLength_50.mat', {'yobs':yobs,
                              'nature':nature,
                              'analysis':analysis,
                              'forecast':forecast,
                              'numstep':numstep,
                              ...
                              'rmse_anl':rmse_anl,
                              'rmse_firstguess':rmse_firstguess,
                              'rmse_obs':rmse_obs,
                              'bias_anl':bias_anl,
                              'bias_firstguess':bias_firstguess,
                              'bias_obs':bias_obs})
