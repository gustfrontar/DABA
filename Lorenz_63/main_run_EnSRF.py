...
da_exp['exp_id']='EnSRF_xyz'    #Este es un identificador que se agregara al nombre de los archivos generados por este script (figuras, pickle contiendo los datos, etc)
...
for i in tqdm( range(1,da_exp['numstep']) ) :
    ...
    for iens in range( da_exp['EnsSize'] ) :
        ...
        for k in range( da_exp['forecast_length'] ) :
            ...
            if ( i + k < da_exp['numstep'] ) :  #Solo para cerciorarme de que 
                                                #el pronostico no queda fuera del rango del experimento.
    ...
    [ da_exp['stateaens'][i,:,:] , da_exp['statea'][i,:] , da_exp['Pa'][i,:,:] , da_exp['OmB'][i,:] , da_exp['OmA'][i,:] ] =da.analysis_update_EnSRF(da_exp['yobs'][i,:],da_exp['statefens'][i,:,:,0],forward_operator,da_exp['R'],da_exp['MultInf'])
