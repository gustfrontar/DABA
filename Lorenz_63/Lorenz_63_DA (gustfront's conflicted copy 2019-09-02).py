...
def JGrad4DVAR( xb , x , parameters , dt , bst , yo , invP , invR , forward_operator , forward_operator_tl ) :
   ...
   J = np.dot( update , np.dot( invP , update ) )
   
   nablaJ = 2 * np.dot( invP , update )
   ...
   for it in range( ntimes ) :
       ...
       for i in range( bst ):
           ...
           L = model.tl_model( xf , parameters , dt ) 
           L_total = np.dot( L , L_total )
            
           xf = model.forward_model( xf , parameters , dt )

       innovation = yo[it,:] - forward_operator( xf )
       
       J = J + np.dot( innovation.np.transpose() , np.dot( invR , innovation ) )
 
       H = forward_operator_tl( xf )
       
       nablaJ = nablaJ - 2*np.dot( L_total.transpose() , np.dot( H.transpose() , innovation ) )
