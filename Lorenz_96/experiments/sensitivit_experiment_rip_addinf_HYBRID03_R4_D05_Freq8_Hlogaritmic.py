...
out_filename='Sesitivity_experiment_rip_addinf_HYBRID03_' + conf.GeneralConf['NatureName'] + '_EnsSize.npz'
...
conf.DAConf['BridgeParam']=0.3                            #Bridging parameter for the hybrid 0-pure LETKF, 1.0-pure ETPF
...
if PlotTheExperiment  :
    ...
    [results,add_inf_range,nrip_range,total_analysis_rmse,total_forecast_rmse,total_analysis_sprd,total_forecast_sprd] = pickle.load(f)
    f.close()
    
    import matplotlib.pyplot as plt 

    plt.pcolormesh(nrip_range,add_inf_range,total_analysis_rmse)
    plt.colorbar()
