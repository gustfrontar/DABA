...
D=np.transpose( ot.emd(np.ones(EnsSize)/EnsSize,w,M,numItermax=1.0e9,log=False) ) * EnsSize

delta = da.riccati_solver( D , w , dt = 0.1 , stop_criteria = 1.0e-3 , iteration_limit = 5000 ) 

delta2 = pf.riccati_solver( ne=EnsSize , d=D , win=w , dt=0.1 , stop_threshold=1.0e-3 , max_iter=5000 )


import matplotlib.pyplot as plt

plt.pcolor(delta);plt.colorbar()
plt.title('DELTA1')
plt.show()

plt.pcolor(delta2);plt.colorbar()
plt.title('DELTA2')
plt.show()

plt.pcolor(delta-delta2);plt.colorbar()
plt.title('DELTA-DELTA2')
plt.show()