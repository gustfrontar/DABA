...
import numpy as np
import time
import letkf_conf_ImperfectModel as conf                #Load the experiment configuration
import os
...
for ie in range(0,NEns)  :
   ...
   for ic in range(0,NCoef) :
      C0[ic,:,ie]=ModelConf['Coef'][ic] + FSpaceAmplitude[ic]*np.cos( 2*np.pi*np.arange(0,Nx)*FSpaceFreq )
...
for it in range( 1 , DALength  )         :
   ...
   [ XFtmp , DF , RFtmp , CRFtmp, CFtmp ]=model.tinteg_rk4( nens=NEns  , nt=DAConf['Freq'] ,  ntout=ntout ,
                                           ...
                                           c0=C0 , crf0=CRF    , cphi=CPhi , csigma=CSigma,
