...
statefens_2=np.zeros((3,EnsSize,1,1))
statefens_2[:,:,0,0]=statefens

print('Using the fortran routine')
M2 = pf.get_distance_matrix( ne=EnsSize , nx=3 , nvar=1 , nt=1 , xens=statefens_2 )

plt.pcolor(M);plt.colorbar()
plt.title('M1')
plt.show()

plt.pcolor(M2);plt.colorbar()
plt.title('M2')
plt.show()

plt.pcolor(M2-M);plt.colorbar()
plt.title('M2-M1')
plt.show()