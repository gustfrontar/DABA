...
da_exp['exp_id']='POEnKF_xyz'    #Este es un identificador que se agregara al nombre de los archivos generados por este script (figuras, pickle contiendo los datos, etc)
...
for i in tqdm( range(1,da_exp['numstep']) ) :
    ...
    [ da_exp['stateaens'][i,:,:] , da_exp['statea'][i,:] , da_exp['Pa'][i,:,:] , da_exp['OmB'][i,:] , da_exp['OmA'][i,:] ] =da.analysis_update_POEnKF(da_exp['yobs'][i,:],da_exp['statefens'][i,:,:,0],forward_operator,forward_operator_tl,da_exp['R'],da_exp['MultInf'])
