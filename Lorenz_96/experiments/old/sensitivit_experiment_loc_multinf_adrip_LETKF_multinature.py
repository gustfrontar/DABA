...
outfile_prefix = './npz/Sesitivity_experiment_loc_multinf_adrip_LETKF_'
...
for Nature in NatureList :
    ...
    total_analysis_rmse = np.zeros( (len(mult_inf_range),len(loc_range),len(rip_range)) )
    total_analysis_sprd = np.zeros( (len(mult_inf_range),len(loc_range),len(rip_range)) )
    total_forecast_rmse = np.zeros( (len(mult_inf_range),len(loc_range),len(rip_range)) )
    total_forecast_sprd = np.zeros( (len(mult_inf_range),len(loc_range),len(rip_range)) )
    ...
    for iinf , mult_inf in enumerate( mult_inf_range ) :
        for iloc , loc in enumerate( loc_range )  :
            for irip , rip in enumerate( rip_range )          :
                ...
                total_analysis_rmse[iinf,iloc,irip] = np.mean(results[-1]['XASRmse'])
                total_forecast_rmse[iinf,iloc,irip] = np.mean(results[-1]['XFSRmse'])
                total_analysis_sprd[iinf,iloc,irip] = np.mean(results[-1]['XASSprd'])
                total_forecast_sprd[iinf,iloc,irip] = np.mean(results[-1]['XFSSprd'])
        
    f=open(out_filename,'wb')
    pickle.dump([results,mult_inf_range,loc_range,rip_range,total_analysis_rmse,total_forecast_rmse,total_analysis_sprd,total_forecast_sprd],f)
    f.close()
