...
#This version includes a local linearization of the h operator. 
...
import numpy as np
import time
import assimilation_conf_GMPerfectModel_R4_Den05_Freq8_Hlogaritmic as conf         #Load the experiment configuration
...
for it in range( 1 , DALength  )         :
   ...
   for itemp in range( DAConf['NTemp'] ) :
      ...
      #Obtain the ensemble mean and perturbations
      [xmean , xpert ]=das.da_mean_and_pert(nx=Nx,nt=1,nvar=1,nens=NEns,xfens=stateens)
      
      YF=np.zeros((NObsW,NEns,NEns))
      YFmask_ens = np.zeros((NObsW,NEns)).astype(bool)
      
      #We will generate NEns ensembles. Each one centered around one of the original ensemble members
      #and will compute Y for each ensemble. YF will now contain an as many Y as the number of ensemble members.
      #This means that we need to apply the observation operator NEns times.
      
      tmp_ens=np.zeros(stateens.shape)
      for ie in range(NEns)   :
          for je in range(NEns) :
             tmp_ens[:,je] = stateens[:,ie] + np.squeeze(xpert[:,je,0,0]) #We recenter the ensemble perturbations around the ie-th ensemble member.
          #print(tmp_ens[0,:])   
          [ YF[:,:,ie] , YFmask_ens[:,ie] ] = hoperator.model_to_obs(  nx=Nx , no=NObsW , nt=1 , nens=NEns ,
                             obsloc=ObsLocW , x=tmp_ens , obstype=ObsTypeW ,
      ...
      [tmp_stateens , weigths] = das.da_gmdr_localh( nx=Nx , nt=1 , no=NObsW , nens=NEns ,  xloc=ModelConf['XLoc']         ,
